...
# data saving folder
SAVE_PATH =  "/MPC_DynamicSys/sharedVol/train_data/nmpc/multi_random"
    ...
    while retries < nMaxGuess:
        try:
            ...
            if retries<1:
                ...
                initial_guess_x += initial_guess_x + np.random.randn(1)[0]
                initial_guess_u += initial_guess_u + np.random.randn(1)[0]
...
def RunMPCForSingle_IniState_IniGuess(x_ini_guess: float, u_ini_guess:float,idx_group_of_control_step:int,x0_state:np.array, 
    ...
    try:
        # normal at x0
        u_sol, x_sol = MPC_NormalData_Process(x0_state, x_ini_guess, u_ini_guess, idx_group_of_control_step, u_result_normal, j_result_normal, x_result_normal)
        ...
        for idx_control_step in range(1, CONTROL_STEPS):
            ...
            u_sol, x_sol = MPC_NormalData_Process(x0_next, x_ini_guess, u_ini_guess, idx_group_of_control_step, u_result_normal, j_result_normal, x_result_normal,idx_control_step)
...
def main():
    ...
    with Manager() as manager:
        ...
        for turn in range(0,num_datagroup):
            # initial guess
            u_ini_guess = np.random.uniform(MIN_U_INIGUESS, MAX_U_INIGUESS, 1)[0]
            if u_ini_guess >=0:
                x_ini_guess = 5
            else:
                x_ini_guess = -5
