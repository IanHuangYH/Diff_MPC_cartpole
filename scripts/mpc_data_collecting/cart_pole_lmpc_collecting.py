...
# Define the initial states range (4 DoF)
rng_x = np.linspace(-3,3,12)                  # position: [-3,3] 12 initial value
rng_x_dot = np.linspace(-3,3,12)              # velocity: [-3,3] 12 initial value
rng_theta = np.linspace(-1,1,10)              # angle: [-1,1] 10 initial value
rng_theta_dot = np.linspace(-np.pi,np.pi,30)  # angular velocity: [-pi,pi] 30 initial value
rng0 = []
for m in rng_x:
    for n in rng_x_dot:
        for p in rng_theta:
...
x0_track = np.zeros((num_datagroup, 4))
x_predicted_track = np.zeros((num_datagroup, N+1, 4))
u_sequence_track = np.zeros((num_datagroup, N, 1))

# data (x,u) collecting (saved in PT file)
x0_tensor = torch.zeros(num_datagroup,4) # x0: 1,440,000*4
x_predicted_tensor = torch.zeros(num_datagroup,N+1,4) # x_predicted: 1,440,000*(8+1)*4
u_sequence_tensor = torch.zeros(num_datagroup,N,1) # u: 1,440,000*8*1
...
for turn in range(0,num_datagroup):
  ...
  x_predicted_track[turn,:,:] = X_sol
  U_sol = sol.value(U_pre)
  print(f'U_sol - {U_sol}')
  u_sequence_track[turn,:,0]= U_sol
  ...
  # save u_sequence_track in tensor
  #   u_reshape = U_sol.reshape(1,N)
  #   u_tensor = torch.tensor(u_reshape)
  #   # print(f'u_tensor_shape -- {u_tensor}')
  #   print(f'turn {turn}')
  #   u_sequence_tensor[turn,:,0] = u_tensor


# saved in tensor
x0_tensor = torch.tensor(x0_track)
x_predicted_tensor = torch.tensor(x_predicted_track)
u_sequence_tensor = torch.tensor(u_sequence_track)
...
print(f'first_u -- {u_sequence_tensor[0,:,0]}')
print(f'first_x_predicted -- {x_predicted_tensor[0,:,:]}')
print(f'first_x0 -- {x0_tensor[0,:]}')

# save u data in PT file for training
torch.save(u_sequence_tensor, os.path.join(folder_path, f'u_sequence_tensor_43200-8-1.pt'))

# save x0 data in PT file as conditional info in training
torch.save(x0_tensor, os.path.join(folder_path, f'x0_tensor_43200-4.pt'))

# save x_predicted data in PT file for possible cost calculation
torch.save(x_predicted_tensor, os.path.join(folder_path, f'x_predicted_tensor_43200-9-4.pt'))
