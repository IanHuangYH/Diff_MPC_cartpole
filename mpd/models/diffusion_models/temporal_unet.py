...
class ConditionedTemporalUnet(nn.Module):
    ...
    def forward(self, x, time, context, context_mask):
        """
        x : [ batch x horizon x state_dim ]
        context: [batch x context_dim]
        """
        ...
        # mask the context
        context_mask = context_mask.repeat(1,context.size(1)) # 1*64 --> 4*64 (4: dim_num)
        # print(f"context_mask_size -- {context_mask.size()}")
        context_mask = (1*(1-context_mask)) # (-1*(1-context_mask)) 
        # print(f"context_size -- {context.size()}")
        context = torch.mul(context, context_mask)
        # print(f"masked-context -- {context}")
        ...
        # swap horizon and channels (state_dim)
        x = einops.rearrange(x, 'b h c -> b c h')  # batch, horizon, channels (state_dim) --> batch, channels, horizon (64,1,8)
        # print(f"x -- {x.shape}")
        # cond_hidden1, cond_hidden2, cond_hidden3 = cond_fns
